...
- gRPC `GetPoints`/`ScrollPoints`/`SearchPoints`/`RecommendPoints`/`DiscoverPoints`/`QueryPoints` ordinary payload read wrappers도 `private-result-oram/v1` payload path를 반환하려 하면 common read guard와 같은 private result ORAM session API 안내로 fail closed 되는지 고정한다.

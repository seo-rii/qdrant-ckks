...
- REST/gRPC snapshot route fixture는 collection/full snapshot creation과 active lifecycle window의 상호배제, collection recovery와 active private ORAM snapshot/upload window의 상호배제, shard snapshot list/create/stream/download/delete/recover, partial snapshot manifest, partial recover_from route가 private ORAM bucket store collection에서 fail closed 되는지도 고정한다. 오류는 submitted snapshot location, operation label, root hash, bucket ciphertext, collection-local private ORAM store path를 반사하지 않는다.

...
- SDK manifest/read_paths/commit signing helpers와 read_paths message builder는 malformed manifest, path-count mismatch, malformed 또는 duplicate read path label/root/hash, non-advancing commit epoch, empty commit을 canonical message construction 전에 거부한다.

...
- panic telemetry와 gRPC status logging redaction도 private ORAM session/path/raw read_paths/access path/read bucket id/bucket sequence/bucket or leaf commitment/entry node/level mask/visited node/neighbor/query vector/score/distance/candidate score/distance/node score/distance/result/token/client-state/client-state ciphertext/hash/sha256/position-map/stash/update bucket/signature, owner/signing key id, signature-public-key registry, access-volume count/length snake_case·camelCase alias sentinel을 반사하지 않는지 검증한다.

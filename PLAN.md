...
- Phase G: cluster parity fingerprint, private-ORAM transfer fail-closed, shard-local epoch ownership, consensus-backed epoch/root CAS를 설계하고 e2e 테스트한다.
  ...
  - private-ORAM cluster gate는 고정된 non-empty shard layout에서 한 shard씩 수동 `move_shard`/`replicate_shard` 또는 automatic dead-replica `replicate_shard`를 열며, source peer에서 명시적 `stream_records`, `to_shard_id=None`으로 실행해야 한다. Collection-global encrypted store는 모든 fully-active shard replica owner의 합집합에 복제된다. Coordinator는 모든 private HNSW/result index에 동일한 hashed consensus lease reservation을 획득하고 target에 current epoch/root/writeback-digest와 전체 encrypted bucket bundle을 exact install한 뒤에만 `private_oram_preinstalled` transfer를 Raft에 제출한다. 활성 reservation 동안 새 session/recovery cleanup은 lease를 조기 해제하지 못하고, transfer marker가 적용된 뒤에는 session open/writeback이 collection state guard에서 닫힌다. 현재 source peer가 요청한 sole active marked transfer의 same-key `restart_transfer`도 새 reservation과 전체 store exact reinstall 뒤 atomic abort/start로 허용하며, consensus apply에서 old marker, no filter, no temporary shard, old/new `stream_records`를 다시 검증한다. Exact single-`Active` replica removal은 남는 owner coordinator가 pending writeback recovery와 전체 index consensus reservation을 완료하고 fully-active fixed layout을 재검증한 뒤 internal marker가 있는 단일 remove를 제출하는 경우에만 허용한다. 제거된 peer의 ciphertext는 보존하지만 active owner union에서 빠졌다면 새 coordinated session을 거부한다. 미표시 또는 method/key가 바뀐 restart, `replicate_points`, snapshot/WAL/resharding transfer, dynamic shard-layout change는 계속 fail closed 한다. Resharding progress, shard-key layout 변경, dead/transitional/batch/final replica removal도 consensus/local apply 전에 차단하고 cleanup용 abort만 허용한다.
  ...
  - Private ORAM resharding의 중간 단계는 ORAM layout을 바꾸지 않는 exact transition만 허용한다. `CommitRead`/`CommitWrite`는 collection의 active reshard key와 stage 검사를 그대로 거치며, replica promotion은 `MigratingPoints`에서 scale-up exact target의 `Resharding -> Active` 또는 같은 shard-key scale-down receiver의 `ReshardingScaleDown -> Active`만 허용한다. Wrong peer/shard/stage/current-state와 그 밖의 transitional state 변경은 collection 계층에서 fail closed다. Active resharding이나 shard transfer가 하나라도 있으면 새 HNSW/result ORAM session coordinator를 거부하므로 start부터 finish까지 consensus epoch/root와 layout index-state digest가 고정된다. Public resharding guard는 별도 활성화 단계 전까지 계속 닫혀 있다.
  - Phase G의 현재 gate는 다음과 같다. Dispatcher consensus state와 모든 shard가 fully active인 고정 layout의 owner-peer union이 있는 REST/gRPC 경로에서는 initial upload와 session open/read/commit/close가 열려 있다. 각 shard의 수동 `stream_records` Move/Replicate, source-coordinated automatic dead-replica recovery, 그리고 남는 owner/replica를 보장하는 reserved exact single-`Active` replica removal은 열려 있다. Consensus coordinator가 없는 distributed TOC, transitional replica layout, unsupported shard transfer, dynamic shard-layout movement, resharding/shard-key 변경, dead/transitional/batch/final replica removal은 계속 fail closed 한다. 위의 “distributed route guard 유지” 문구들은 각 하위 primitive 구현 당시의 단계 기록이며 이 현재 gate가 최종 상태를 정의한다.

...
- Common/gRPC read fixtures는 private result ORAM collection에서도 payload를 요청하지 않는 허용 경로를 열어 두어, payload-omitted retrieve/search 요청이 private result ORAM session을 요구하지 않는지도 고정한다.

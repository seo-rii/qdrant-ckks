...
- Snapshot/client-state, slow-log/request-hash redaction, panic/error-reporting/audit redaction, metrics label, REST/gRPC route, receiving-shard, replica-priority recovery, and consensus transfer guards now treat `client_state_backup`, `encrypted_client_state`/`encryptedClientState`, encrypted client-state backup, `*_ciphertext_sha256`, position-map/ORAM-position-map backup, token-position-map backup, and stash backup variants like other client-owned ORAM state aliases, so client backup material cannot be snapshotted as server-owned ORAM files or leak through slow/error/status logs, request hashes, metrics labels, route errors, or storage-level recovery/transfer errors.

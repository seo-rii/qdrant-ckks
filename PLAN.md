...
- Snapshot/client-state and log redaction alias guards now treat `encrypted_client_state`/`encryptedClientState` and plural forms like other client-owned ORAM state aliases, so encrypted client-state backups cannot be snapshotted as server-owned ORAM files or leak through slow/error/status logs.

...
- Private HNSW ORAM productionization: read-only bulk-built consensus-backed single-writer MVP, result ORAM fetch path, single-shard manual `stream_records` Move/Replicate preinstall, ReplicateShard/MoveShard process E2E, partial live-preinstall failure/retry, active transfer abort/retry, 63-bucket large-bundle process benchmark는 들어갔지만 dynamic online HNSW insertion, multi-writer ownership, multi-shard/automatic/restart transfer, resharding, and chunked internal transport는 future work다.
...
- Phase G: cluster parity fingerprint, private-ORAM transfer fail-closed, shard-local epoch ownership, consensus-backed epoch/root CAS를 설계하고 e2e 테스트한다.
  ...
  - private-ORAM cluster gate는 단일 shard collection의 수동 `move_shard`/`replicate_shard`만 열며, source peer에서 명시적 `stream_records`, `to_shard_id=None`으로 요청해야 한다. Coordinator는 모든 private HNSW/result index에 동일한 hashed consensus lease reservation을 획득하고 target에 current epoch/root/writeback-digest와 전체 encrypted bucket bundle을 exact install한 뒤에만 `private_oram_preinstalled` transfer를 Raft에 제출한다. 활성 reservation 동안 새 session/recovery cleanup은 lease를 조기 해제하지 못하고, transfer marker가 적용된 뒤에는 session open/writeback이 collection state guard에서 닫힌다. 미표시 transfer와 `replicate_points`, `restart_transfer`, snapshot/WAL/resharding transfer, multi-shard layout, 자동 dead-replica recovery는 계속 fail closed 한다. Resharding progress, shard-key layout 변경, replica removal도 기존대로 consensus/local apply 전에 차단하고 cleanup용 abort만 허용한다.
  ...
  - 별도 typed `InstallPrivateOramLiveReplica` internal RPC와 ChannelService source transport가 HNSW/result live bundle에 연결됐다. Receiver는 wire aggregate bound, local Raft ownership epoch/root/digest exact match를 확인하고, active consensus lease가 있으면 shard-transfer coordinator가 보낸 canonical reservation hash와 정확히 일치할 때만 node-local private-ORAM mutation lock 아래 provider signature/full-store install을 수행한다. Source helper도 export 결과와 local Raft record를 대조하고 target ACK의 epoch/root/digest exact match를 요구하며 transport/ACK 오류는 peer id 외 root/digest/ciphertext를 반사하지 않는다. Public cluster update는 지원되는 single-shard 수동 Move/Replicate에서 이 helper를 호출해 preinstall ACK 뒤 marked transfer를 제출한다.
  - Phase G의 현재 gate는 다음과 같다. Dispatcher consensus state와 uniform fully-active replica set이 있는 REST/gRPC 경로에서는 initial upload와 session open/read/commit/close가 열려 있다. 단일 shard 수동 `stream_records` Move/Replicate는 source-coordinated reservation/preinstall 뒤 열려 있다. Consensus coordinator가 없는 distributed TOC, transitional/mismatched replica layout, unsupported shard transfer, resharding/shard-key 변경, replica removal은 계속 fail closed 한다. 위의 “distributed route guard 유지” 문구들은 각 하위 primitive 구현 당시의 단계 기록이며 이 현재 gate가 최종 상태를 정의한다.

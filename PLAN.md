...
- Phase G: cluster parity fingerprint, private-ORAM transfer fail-closed, shard-local epoch ownership, consensus-backed epoch/root CAS를 설계하고 e2e 테스트한다.
  ...
  - Resharding layout foundation은 stable pre-layout과 `ReshardKey`에서 scale-up/scale-down 이후 canonical shard-layout digest와 owner union을 계산하는 fail-closed helper로 시작했다. Scale-up의 기존 shard id 재사용, scale-down의 missing shard/wrong owner/shard-key mismatch, 마지막 auto shard 또는 custom shard-key의 마지막 shard 제거는 거부한다. `PrivateOramReshardingLayoutTransition`은 exact reshard key, expected/new generation과 digest, target shard owner set, index epoch/root/writeback state를 묶고 scale-up/down pre/post topology를 replay-safe하게 분류하며 Debug에서는 collection/index/root/shard-key를 redaction한다. Persistent validator는 typed start/finish envelope의 모든 index에 동일한 exact consensus lease가 잡혀 있고 canonical 순서의 epoch/root/writeback state와 layout index-state digest가 현재 Raft state에 일치하는지 검증한다. Start는 expected layout에서만, finish는 expected 또는 idempotently applied new layout에서만 허용된다. 이 계약이 target full-store preinstall과 consensus apply orchestration에 연결되기 전까지 public resharding gate는 계속 닫혀 있다.

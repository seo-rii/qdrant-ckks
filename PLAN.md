...
- Snapshot/client-state and log redaction alias guards now treat `client_state_backup`, `encrypted_client_state`/`encryptedClientState`, encrypted client-state backup, position-map backup, token-position-map backup, and stash backup variants like other client-owned ORAM state aliases, so client backup material cannot be snapshotted as server-owned ORAM files or leak through slow/error/status logs.

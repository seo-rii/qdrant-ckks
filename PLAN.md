...
- Private HNSW ORAM productionization: read-only bulk-built consensus-backed single-writer MVP, result ORAM fetch path, single-shard manual `stream_records` Move/Replicate preinstall과 ReplicateShard/MoveShard process E2E는 들어갔지만 dynamic online HNSW insertion, multi-writer ownership, multi-shard/automatic/restart transfer, resharding, transfer fault-injection process coverage, and large-bundle benchmarks는 future work다.

...
- Phase G: cluster parity fingerprint, private-ORAM transfer fail-closed, shard-local epoch ownership, consensus-backed epoch/root CAS를 설계하고 e2e 테스트한다.
  ...
  - 실제 single-peer Raft proposal/apply 루프를 사용하는 HNSW 통합 테스트는 분산 create가 생성한 stable collection UUID로 client key/bucket/manifest를 다시 바인딩한 뒤 initial ownership CAS, session lease acquire, signed read, digest-bound commit, consensus epoch 확인, close lease release를 순서대로 검증한다. Dispatcher의 별도 replicated-writeback fixture가 exact remote ACK set과 remote-before-local finalize를 검증한다. 남은 Phase G 검증 공백은 여러 실제 peer process를 통과하는 public route/fault-injection E2E와 result ORAM의 같은 actual-Raft owner flow다.

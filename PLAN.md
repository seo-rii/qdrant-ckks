...
- Private HNSW ORAM productionization: read-only bulk-built consensus-backed single-writer MVP, result ORAM fetch path, single-shard manual `stream_records` Move/Replicate preinstall은 들어갔지만 dynamic online HNSW insertion, multi-writer ownership, multi-shard/automatic/restart transfer, resharding, and broader multi-node transfer/benchmark coverage는 future work다.

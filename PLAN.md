...
- manifest-aware `sign_private_hnsw_oram_read_paths_for_manifest` helper는 manifest epoch/root/key lineage/owner signing key, `oram.path_batch_size`, tree-bounded leaf label을 읽기 서명 context로 사용해 fixed read batch 수나 leaf range가 맞지 않으면 SDK에서 서명 전에 fail closed 한다.

...
- REST/gRPC private result ORAM live fixture도 active result ORAM session이 있는 collection의 collection/full snapshot 생성을 거부하고, 오류가 result root hash, bucket ciphertext, collection-local `private_result_oram` path를 반사하지 않는지 검증한다.

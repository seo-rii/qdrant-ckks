...
- Phase G: cluster parity fingerprint, private-ORAM transfer fail-closed, shard-local epoch ownership, consensus-backed epoch/root CAS를 설계하고 e2e 테스트한다.
  ...
  - Storage consensus apply guard 오류도 private HNSW/result ORAM runtime key id, rule id, instance id, binding id 같은 config sentinel을 반사하지 않는지 transfer/resharding/shard-key/replica-remove fixture로 고정한다.
...
- SDK manifest/read_paths/commit signing helpers는 malformed manifest, path-count mismatch, malformed 또는 duplicate read path label/root/hash, non-advancing commit epoch, empty commit을 canonical message construction 전에 거부한다.
...
- panic telemetry와 gRPC status logging redaction도 private ORAM session/path/raw read_paths/access path/read bucket id/bucket sequence/bucket or leaf commitment/entry node/level mask/visited node/neighbor/query vector/score/distance/candidate score/distance/node score/distance/result/token/client-state/position-map/stash/update bucket/signature snake_case·camelCase alias sentinel을 반사하지 않는지 검증한다.
...
- Common update batch operation fixture도 private HNSW ORAM `Upsert`/`Delete`/`DeleteVectors`와 private result ORAM `Upsert`/`OverwritePayload`/`DeletePayload`/`ClearPayload`/`Delete`가 direct ordinary update guard와 같은 session API 안내로 fail closed 되는지 고정한다.

...
- manifest-aware `sign_private_hnsw_oram_read_paths_for_manifest` helper는 manifest epoch/root/key lineage/owner signing key와 `oram.path_batch_size`를 읽기 서명 context로 사용해 fixed read batch 수가 맞지 않으면 SDK에서 서명 전에 fail closed 한다.
...
- manifest-aware `sign_private_result_oram_read_buckets_for_manifest` helper는 manifest epoch/root/key lineage/owner signing key와 `oram.path_batch_size * (oram.tree_height + 1)` fixed bucket-id volume을 읽기 서명 context로 사용해 fixed read batch 수가 맞지 않으면 SDK에서 서명 전에 fail closed 한다.

...
- panic telemetry와 gRPC status logging redaction도 private ORAM session/path/raw read_paths/access path/read bucket id/bucket sequence/bucket or leaf commitment/entry node/level mask/visited node/neighbor/query vector/candidate score/distance/node score/distance/result/token/client-state/position-map/stash/update bucket/signature snake_case·camelCase alias sentinel을 반사하지 않는지 검증한다.

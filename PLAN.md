...
- Private HNSW ORAM productionization: read-only bulk-built consensus-backed single-writer MVP와 result ORAM fetch path는 들어갔지만 dynamic online HNSW insertion, multi-writer ownership, private ORAM bucket shard transfer/resharding, and broader multi-node benchmark coverage는 future work다.
...
- Phase G: cluster parity fingerprint, private-ORAM transfer fail-closed, shard-local epoch ownership, consensus-backed epoch/root CAS를 설계하고 e2e 테스트한다.
  ...
  - 별도 typed `InstallPrivateOramLiveReplica` internal RPC와 ChannelService source transport가 HNSW/result live bundle에 연결됐다. Receiver는 wire aggregate bound를 먼저 적용하고 local Raft ownership epoch/root/digest exact match와 active consensus session lease 부재를 확인한 뒤 node-local private-ORAM mutation lock 아래 provider signature/full-store install을 수행한다. Source helper도 export 결과와 local Raft record를 대조하고 target ACK의 epoch/root/digest exact match를 요구하며 transport/ACK 오류는 peer id 외 root/digest/ciphertext를 반사하지 않는다. Ordinary shard-transfer guard는 아직 이 helper를 호출하지 않으며, 다음 단계는 single-shard v1 preinstall → shard data transfer → membership finish 순서에 연결하는 것이다.
  - Phase G의 현재 gate는 다음과 같다. Dispatcher consensus state와 uniform fully-active replica set이 있는 REST/gRPC 경로에서는 initial upload와 session open/read/commit/close가 열려 있다. Consensus coordinator가 없는 distributed TOC, transitional/mismatched replica layout, shard transfer/resharding/shard-key 변경, replica removal은 계속 fail closed 한다. 위의 “distributed route guard 유지” 문구들은 각 하위 primitive 구현 당시의 단계 기록이며 이 현재 gate가 최종 상태를 정의한다.

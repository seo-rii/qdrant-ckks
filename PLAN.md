...
- private HNSW ORAM vector store 이름은 `client.state`, `position.map`, `stash`처럼 separator를 제거하면 client-owned state alias가 되는 값을 거부하고, snapshot source/restore preflight도 확장자 없는 dotted alias와 확장자 포함 alias를 같은 client-owned state로 fail closed 한다.

...
- REST/gRPC ORAM session fixture는 active private HNSW ORAM session이 있는 collection의 collection/full snapshot 생성도 거부하고, 오류가 root hash나 collection-local `private_hnsw_oram` path를 반사하지 않는지 검증한다.

...
- Private HNSW ORAM productionization: read-only bulk-built consensus-backed single-writer MVP, result ORAM fetch path, exact single-shard manual/automatic `stream_records` preinstall, ReplicateShard/MoveShard/automatic recovery process E2E, partial live-preinstall failure/retry, active transfer abort/retry, 63-bucket large-bundle process benchmark는 들어갔지만 dynamic online HNSW insertion, multi-writer ownership, multi-shard/restart transfer, resharding, and chunked internal transport는 future work다.
...
- Phase G: cluster parity fingerprint, private-ORAM transfer fail-closed, shard-local epoch ownership, consensus-backed epoch/root CAS를 설계하고 e2e 테스트한다.
  ...
  - private-ORAM cluster gate는 단일 shard collection의 수동 `move_shard`/`replicate_shard`와 automatic dead-replica `replicate_shard`만 열며, source peer에서 명시적 `stream_records`, `to_shard_id=None`으로 실행해야 한다. Coordinator는 모든 private HNSW/result index에 동일한 hashed consensus lease reservation을 획득하고 target에 current epoch/root/writeback-digest와 전체 encrypted bucket bundle을 exact install한 뒤에만 `private_oram_preinstalled` transfer를 Raft에 제출한다. 활성 reservation 동안 새 session/recovery cleanup은 lease를 조기 해제하지 못하고, transfer marker가 적용된 뒤에는 session open/writeback이 collection state guard에서 닫힌다. 미표시 transfer와 `replicate_points`, `restart_transfer`, snapshot/WAL/resharding transfer, multi-shard layout은 계속 fail closed 한다. Resharding progress, shard-key layout 변경, replica removal도 기존대로 consensus/local apply 전에 차단하고 cleanup용 abort만 허용한다.

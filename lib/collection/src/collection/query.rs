...
use crate::config::{
    EncryptionRuleRef, EncryptionSelector, encryption_rule_uses_private_hnsw_oram,
    ...
    async fn do_query_batch(
        ...
        if metadata_required && is_required_transfer_large_enough {
            ...
            let redacted_plan = if encrypted_payload_read_modes
                .iter()
                .any(|mode| *mode == segment::types::EncryptedPayloadReadMode::Redacted)
            {
                self.encrypted_payload_redaction_plan_for_mode(
            ...
            let raw_plan = if encrypted_payload_read_modes
                .iter()
                .any(|mode| *mode == segment::types::EncryptedPayloadReadMode::Raw)
            {
                self.encrypted_payload_redaction_plan_for_mode(
            ...
            for (points, mode) in result.iter_mut().zip(encrypted_payload_read_modes) {
                let redaction_plan = match mode {
                    segment::types::EncryptedPayloadReadMode::Redacted => redacted_plan.as_ref(),
                    segment::types::EncryptedPayloadReadMode::Raw => raw_plan.as_ref(),
                    _ => None,
                };
...
fn encrypted_direct_query_rule_error(
...
mod tests {
    ...
    fn private_hnsw_query_encryption(vector_name: &str) -> CollectionEncryptionConfig {
    ...
    fn encrypted_direct_query_error(
        ...
        for rule in &encryption.rules {
            ...
            if names.iter().any(|name| name == vector_name) {
                return Some(encrypted_direct_query_rule_error(rule, vector_name));
            }
        }
    ...
    fn private_hnsw_direct_query_error_uses_session_api_without_vector_name() {
        ...
        let err = encrypted_direct_query_error(&encryption, private_vector)
            .expect("private HNSW direct query must be rejected");

...
supplied by the client. Legacy `average_vector` recommend also supports exactly
one positive point-id example and no negatives by loading that point's stored
sidecar ciphertext as the query; point-id recommend examples in other

...
smaller-is-better. This executor is intentionally brute-force: it does not use
HNSW pruning, quantization, ACORN/indexed-only params, recommend/discover vector
arithmetic, search-groups/query-groups, search matrix, prefetch/fusion/MMR, or
encrypted query ciphertexts. Direct collection-internal calls without runtime
settings still fail closed for encrypted vector names. The

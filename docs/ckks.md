...
Non-fusion encrypted prefetch, quantization, ACORN/indexed-only params, or
client-supplied encrypted query ciphertexts remain unsupported. Root direct
`NearestWithMmr` over an encrypted vector name uses CKKS sidecar scoring for
both query-to-candidate relevance and candidate-to-candidate diversity; it is
limited to large-better metrics (`dot`/`cosine`). Legacy and

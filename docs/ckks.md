...
REST universal `query`/`query/groups` and REST legacy `search`/`search/groups`
nearest-neighbor requests may also provide a client-supplied CKKS encrypted
query ciphertext envelope. Qdrant
...
ACORN/indexed-only params and gRPC legacy `SearchPoints` client-encrypted query

...
| `retrieve`, `scroll`, `search`, and `query` result payloads | Stored `$qdrant_sec` markers are returned raw. There is no `decrypt_payload` option or RBAC capability yet. | Stored `$qdrant_client_aead` markers are returned raw for SDK/client decryption. | Stored vector sidecar payload envelopes are returned raw when payloads are requested. Read/search/query/recommend/discover paths, including grouped variants, reject `with_vector=true` or selectors that request encrypted vector names; clients must request the payload sidecar instead. REST/gRPC nearest-neighbor dense-vector `search`, `search/groups`, root direct `query`, root direct point-id nearest `query`, root direct `query/groups`, root direct point-id nearest `query/groups`, `search/matrix`, raw-dense `recommend` (`average_vector`, `best_score`, `sum_scores`), point-id recommend for single-positive `average_vector` and for `best_score`/`sum_scores`, legacy `discover` with raw-dense or point-id target/context examples, and universal `discover`/`discover groups`/`context`/`context groups` queries with raw-dense or point-id target/context examples are supported with runtime OpenFHE settings. Nearest-neighbor requests may set `hnsw_ef` to use the ciphertext sidecar candidate graph for raw dense query vectors and stored point-id query vectors; exact requests and non-HNSW requests use brute-force sidecar scoring. Matrix requests sample stored sidecars and score pairwise stored ciphertexts. Quantization/ACORN/indexed-only search params, multi-example point-id `average_vector`, prefetch/fusion/MMR, encrypted query vectors, and mixed encrypted/plaintext batch search remain unsupported. |
...
client-supplied encrypted query ciphertexts remain unsupported. Legacy and
...
supplied by the client. `average_vector` recommend and recommend/groups also
support exactly one positive point-id example and no negatives by loading that
point's stored sidecar ciphertext as the query. `best_score` and `sum_scores`
support raw dense and point-id positive/negative examples because each example
is scored independently before the objective combines the scores. Multi-example
point-id `average_vector` remains unsupported because it would require
encrypted vector arithmetic. Raw-dense `average_vector` is reduced to one
plaintext query vector which is encrypted through the bridge before scoring; `best_score` and

...
sidecar scorer also handles
root direct `query` and `query/groups` requests may also use a point id as the
nearest-neighbor query; Qdrant loads that point's stored CKKS sidecar envelope
...
client-held CKKS search, score decryption, or native Qdrant segment `HNSWIndex`
storage for CKKS ciphertexts. The experimental sidecar HNSW graph can be cached
on disk by ciphertext fingerprint, but it remains separate from Qdrant's
segment-native vector index files.
...
- recommend/discover/context flows that require point-id examples, client-supplied
...
| Payload indexes, filters, facets, ordering, grouping, and formulas | Plaintext indexes, read/update filters, facet keys, order-by keys, group-by keys, and formula payload references over encrypted paths, parent paths, or child paths are rejected. Exact-match search can use a separate client-generated blind-index token field configured through `metadata/blind-index-hmac@v1`; encrypted content itself is not indexed. | Same policy. The opaque ciphertext field is not searchable, orderable, groupable, facetable, or usable in mutation filters as plaintext. Exact-match search requires a separate blind-index token field. | The reserved `$qdrant_sec_vectors` sidecar field is not indexable, filterable, orderable, groupable, facetable, or usable in formulas. Payload filtering/faceting over encrypted metadata is unsupported. |
| `retrieve`, `scroll`, `search`, and `query` result payloads | Stored `$qdrant_sec` markers are returned raw. There is no `decrypt_payload` option or RBAC capability yet. | Stored `$qdrant_client_aead` markers are returned raw for SDK/client decryption. | Stored vector sidecar payload envelopes are returned raw when payloads are requested. Read/search/query/recommend/discover paths, including grouped variants, reject `with_vector=true` or selectors that request encrypted vector names; clients must request the payload sidecar instead. REST/gRPC nearest-neighbor dense-vector `search`, `search/groups`, root direct `query`, root direct point-id nearest `query`, root direct `query/groups`, root direct point-id nearest `query/groups`, `search/matrix`, raw-dense `recommend` (`average_vector`, `best_score`, `sum_scores`), legacy single-positive point-id `average_vector` recommend and recommend/groups, raw-dense `discover`, legacy point-id target discover with raw-dense context pairs, and raw-dense universal `context` queries are supported with runtime OpenFHE settings. Nearest-neighbor requests may set `hnsw_ef` to use the ciphertext sidecar candidate graph for raw dense query vectors and stored point-id query vectors; exact requests and non-HNSW requests use brute-force sidecar scoring. Matrix requests sample stored sidecars and score pairwise stored ciphertexts. Quantization/ACORN/indexed-only search params, point-id recommend examples beyond the single-positive `average_vector` case, point-id discover context pairs, point-id universal context examples, prefetch/fusion/MMR, encrypted query vectors, and mixed encrypted/plaintext batch search remain unsupported. |
...
| Metadata encryption | Metadata value encryption is not implemented. `metadata_keys` selectors are accepted only for client-generated exact-match blind-index token fields with `metadata-exact-match-token/v1`. | Same token-field policy. Qdrant stores opaque blind-index tokens and never computes them. | Metadata encryption is not implemented. |
...
| HNSW graph and quantization | CKKS ciphertext vectors are searched through sidecar ciphertext scoring, not through plaintext dense vector storage. | REST/gRPC nearest-neighbor search can score stored CKKS ciphertext envelopes through the OpenFHE bridge using the collection distance metric. `hnsw_ef` enables an experimental ciphertext sidecar candidate graph for nearest-neighbor queries; exact and non-HNSW requests use brute force. The sidecar graph is cached in memory and persisted under the collection directory by stable crypto identity plus ciphertext fingerprint for restart reuse; persisted graphs must be private, reciprocal, and connected. Raw-dense recommend and raw-dense discover use the same encrypted-query sidecar scoring path but remain brute-force. Quantization remains unsupported for encrypted vectors. | Broaden distributed rebuild/recovery coverage before treating it as a production-grade segment-native HNSW index. |
...
the cache directory must be a private non-symlink directory, cache files must be
private regular files, oversized files are rejected, and metadata/fingerprint
...
vector sampler. Quantization, ACORN/indexed-only params, prefetch/fusion/MMR, or
client-supplied encrypted query ciphertexts remain unsupported. Legacy `recommend` and
universal recommend queries are supported for `average_vector`, `best_score`,
and `sum_scores` when every positive/negative example is a raw dense vector
supplied by the client. Legacy `average_vector` recommend and recommend/groups
also support exactly one positive point-id example and no negatives by loading
that point's stored sidecar ciphertext as the query; point-id recommend examples in other
strategies remain unsupported because they require additional encrypted vector
arithmetic semantics. Raw-dense `average_vector` is reduced to one plaintext
query vector which is encrypted through the bridge before scoring; `best_score` and
...
bridge exposes raw similarity scores for those metrics. Legacy `discover` and
universal discover queries are supported when the target and every context pair
are raw dense vectors supplied by the client. Legacy discover also supports a
point-id target with raw-dense context pairs by loading the target point's stored
sidecar ciphertext; point-id context pairs remain unsupported because they need
additional encrypted context arithmetic semantics. Discover sidecar scoring uses
the same rank plus scaled-sigmoid target objective as Qdrant's plaintext
discover path. Universal `context` queries are supported for raw dense context
pairs and use the same pair-rank objective as Qdrant's plaintext context path.

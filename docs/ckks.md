...
query. Legacy `search`, gRPC query schema support, score decryption, and native
Qdrant segment `HNSWIndex` storage for
...
- legacy `search` and gRPC query schema for client-supplied encrypted query
  ciphertexts
...
and gRPC client-encrypted query schema support remain unsupported. Root direct

...
and server scoring, including ordinary query/fusion, recommend/discover,
grouped search/query, and search matrix paths, fail closed and direct clients
to the private HNSW ORAM session APIs. Phase 11 implements the encrypted bucket
store and session read/commit APIs behind this validated control-plane
contract.
...
submitted key id. Manifest-store layout failures during upload are sanitized
without exposing collection-local `private_hnsw_oram` filesystem paths.

...
| Payload indexes, filters, and facets | Plaintext indexes, filters, and facet keys over encrypted paths, parent paths, or child paths are rejected. Searching or aggregating encrypted content requires a future blind-index provider. | Same policy. The opaque ciphertext field is not searchable or facetable as plaintext. | Payload filtering/faceting over encrypted metadata is unsupported. |

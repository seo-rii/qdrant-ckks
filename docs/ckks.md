...
The `sec` branch adds a small `qdrant-crypto` workspace crate for encrypted
...
| Shard transfer and replication | Sender and receiver must have matching crypto runtime material and CKKS context. | App and distributed peer telemetry expose a non-secret crypto runtime capability fingerprint. Until enforcement is wired into transfer orchestration, encrypted collection data-movement cluster operations fail closed and automatic dead-replica recovery skips encrypted transfers. | Replace the temporary fail-closed transfer gate with cluster capability parity checks and transfer tests. |

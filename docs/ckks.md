...
| Shard transfer and replication | Sender and receiver must have matching crypto runtime material and CKKS context. | App and distributed peer telemetry expose a non-secret crypto runtime capability fingerprint. Until enforcement is wired into transfer orchestration, encrypted collection data-movement cluster operations fail closed. | Replace the temporary fail-closed transfer gate with cluster capability parity checks and transfer tests. |

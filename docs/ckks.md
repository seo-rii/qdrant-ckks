...
| Shard transfer / replication | Automatic encrypted shard transfer fails closed until cluster crypto runtime parity enforcement is wired. | Same policy; client-envelope verifier policy must match across nodes before encrypted transfers are allowed. | Same policy; matching OpenFHE context and metadata AEAD material must be enforced before encrypted transfers are allowed. |
...
| Shard transfer and replication | Sender and receiver must have matching crypto runtime material and CKKS context. | App and distributed peer telemetry expose a non-secret crypto runtime capability fingerprint. Until enforcement is wired into transfer orchestration, encrypted collection shard transfer start fails closed. | Replace the temporary fail-closed transfer gate with cluster capability parity checks and transfer tests. |

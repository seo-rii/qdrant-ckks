...
| Snapshots | Snapshot archives should contain encrypted payload/vector envelopes and enough metadata to preflight required keys/context. | Payload sentinel leakage scan now creates and scans a collection snapshot archive. Collection and shard snapshot recover paths preflight runtime crypto settings for missing instance/material/backend. Wrong key/context restore tests are still missing. | Add restore tests for wrong keys, wrong key ids, and wrong CKKS context. |

...
The gRPC facet, scroll filter/order-by, formula query, grouped search, and
grouped query wrappers enforce the same selector guard after converting their
payload paths.
...
id sequences, updated bucket writebacks, client-state fields, and

...
query. Legacy `search`, score decryption, and native Qdrant segment
`HNSWIndex` storage for
...
- legacy `search` schema for client-supplied encrypted query ciphertexts
...
ACORN/indexed-only params and legacy `search` client-encrypted query
ciphertexts remain unsupported. Root direct

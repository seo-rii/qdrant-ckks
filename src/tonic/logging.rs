    ...
    fn grpc_status_log_message_redacts_private_oram_access_pattern_fields() {
        let status = tonic::Status::invalid_argument(
            "private ORAM read failed for session_id=session-sentinel \
             ...
             payload.len=payload-len-dot-sentinel \
             ...
             bucket.id.sequence=bucket-id-sequence-dot-sentinel \
             ...
             bucket.sequence=bucket-sequence-short-dot-sentinel \
             ...
             point.token=point-token-dot-singular-sentinel \
             point_tokens=point-token-snake-sentinel \
             point.tokens=point-token-dot-sentinel \
             ...
             bucket.commitments=bucket-commitment-dot-plural-sentinel \
        ...
        assert!(!rendered.contains("client-state-snapshots-snake-sentinel"));
        ...
        assert!(!rendered.contains("encrypted-client-state-snapshots-snake-sentinel"));
        ...
        assert!(!rendered.contains("encrypted-client-state-ciphertext-sha256-camel-sentinel"));
        assert!(!rendered.contains("encrypted-client-state-ciphertexts-sha256-camel-sentinel"));
